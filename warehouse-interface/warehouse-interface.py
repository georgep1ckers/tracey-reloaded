...
def generate_random_order():
    """Introduce errors at a 1 in 20 occurence"""
...
def delete_order_from_order_processor(order_id):
    ...
    with tracer.start_as_current_span("WarehouseInterface: Delete Order from Processor", span_kind=trace.SpanKind.SERVER):
...
def get_order_from_order_processor():
    with tracer.start_as_current_span("WarehouseInterface: Get Order from Processor", span_kind=trace.SpanKind.SERVER):
...
def add_order_to_order_processor(order_data):
    with tracer.start_as_current_span("WarehouseInterface: Add Order to Processor", span_kind=trace.SpanKind.SERVER):
...
def check_stock_from_stock_processor(product):
    with tracer.start_as_current_span("WarehouseInterface: Check Stock from stock-controller", span_kind=trace.SpanKind.SERVER):
...
def increase_stock_from_stock_processor(product, quantity):
    with tracer.start_as_current_span("WarehouseInterface: Increase Stock from stock-controller", span_kind=trace.SpanKind.SERVER):
...
def decrease_stock_from_stock_processor(product, quantity):
    with tracer.start_as_current_span("WarehouseInterface: Decrease Stock from stock-controller", span_kind=trace.SpanKind.SERVER):
