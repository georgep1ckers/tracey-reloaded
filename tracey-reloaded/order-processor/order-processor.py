...
def check_orders():
    with tracer.start_as_current_span("OrderProcessor: Check Orders"):
        ...
        try:
            ...
            logging.info(f"Fetched unprocessed orders: {response_data}")
...
def add_orders():
    with tracer.start_as_current_span("OrderProcessor: Add Orders"):
        ...
        try:
            ...
            logging.info(f"Inserted new order with order_id: {order_id}")
            conn.commit()
    ...
    with tracer.start_as_current_span("OrderProcessor: Delete Order"):
        ...
        try:
            ...
            if cursor.rowcount == 0:
                logging.info("No order found to delete.")
                return jsonify({"message": "Order not found"}), 404
            logging.info(f"Deleted order with order_id: {order_id}")
            conn.commit()
